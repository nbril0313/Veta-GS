...
class MiniCam:
    def __init__(self, c2w, width, height, fovy, fovx, znear, zfar, fid):
        ...
        self.world_view_transform = torch.tensor(w2c).transpose(0, 1).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
        ...
        self.camera_center = -torch.tensor(c2w[:3, 3]).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))


class GUI:
    def __init__(self, args, dataset, opt, pipe, testing_iterations, saving_iterations, device) -> None:
                            ...
                            if self.node_vis_fea or True:
                                ...
                                if True:  #self.renderer.gaussians.motion_model.soft_edge:
                                    if hasattr(self.renderer.gaussians.motion_model, 'nodes_fea'):
                                        ...
                                        self.node_rgb = torch.from_numpy(node_rgb).to(self.device)
                                    else:
                                        self.node_rgb = None
                                else:
                                    self.node_rgb = None
                                ...
                                node_rgb = torch.zeros([node_imp.shape[0], 3], dtype=torch.float32).to(self.device)
                                node_rgb[..., 0] = node_imp
    ...
    def train_step(self):
        ...
        if self.iteration < self.opt.warm_up:
            ...
            ast_noise = 0 if self.dataset.is_blender else torch.randn(1, 1, device=self.device).expand(N, -1) * time_interval * self.smooth_term(self.iteration)
    ...
    def test_step(self):
        ...
        cur_cam = MiniCam(
            ...
            self.cam.near,
            self.cam.far,
            fid=torch.remainder(torch.tensor((time.time()-self.t0) * self.fps_of_fid).float().to(self.device) / len(self.scene.getTrainCameras()), 1.)
        )
        fid = cur_cam.fid
...
if __name__ == "__main__":
    ...
    gui = GUI(args=args, dataset=lp.extract(args), opt=op.extract(args), pipe=pp.extract(args),testing_iterations=args.test_iterations, saving_iterations=args.save_iterations, device=device)
