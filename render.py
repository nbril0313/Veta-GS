...
def render_set(model_path, load2gpu_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform, device):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        d_xyz, d_rotation, d_scaling = deform.step(gaussians.get_xyz.detach(), time_input, cam_pos, view_dir)
...
def interpolate_time(model_path, load2gpt_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform, device):
    ...
    for t in tqdm(range(0, frame, 1), desc="Rendering progress"):
        ...
        d_xyz, d_rotation, d_scaling = deform.step(xyz.detach(), time_input, cam_pos, view_dir)
        results = render(view, gaussians, pipeline, background, d_xyz, d_rotation, d_scaling, device, is_6dof)
        rendering = results["render"]
...
def interpolate_view(model_path, load2gpt_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, timer, device):
    ...
    for i, pose in enumerate(tqdm(render_poses, desc="Rendering progress")):
        ...
        d_xyz, d_rotation, d_scaling = timer.step(xyz.detach(), time_input, cam_pos, view_dir)
        results = render(view, gaussians, pipeline, background, d_xyz, d_rotation, d_scaling, device, is_6dof)
        rendering = results["render"]
        renderings.append(to8b(rendering.cpu().numpy()))
        depth = results["depth"]
...
def interpolate_all(model_path, load2gpt_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform, device):
...
def interpolate_poses(model_path, load2gpt_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, timer, device):
